; introduced organism-specific parameters on 5/15
...
; nurses and patients own "handcontamCdiff" or contamCdiffination per hand
nurses-own[location contamCdiff contamMRSA AH]
...
;wheelchairs-own [contamCdiff1 contamCdiff2 contamMRSA1 contamMRSA2 AWhandle AWwheel contamCdifftemp contamMRSAtemp cleaned?]
wheelchairs-own [contamCdiff1 contamCdiff2 contamMRSA1 contamMRSA2 AWwheel contamCdifftemp contamMRSAtemp cleaned?]
...
to setup
  ...
  ; 1. Acute
    ...
    ask zone1-patches [
      ...
      let contamprob random-float 1
      ifelse contamprob <= 0.03 ; probability that surface is contaminated
    [set contamMRSA3 random-float 0]
    [set contamMRSA3 0]
      let contamprob1 random-float 1
    ifelse contamprob1 <= 0.1435
      [set contamCdiff3 random-float 0]
      [set contamCdiff3 0]
    ifelse contamCdiff3 > 0.4
      [set pcolor scale-color red contamCdiff3 .5 0]
    [set pcolor white]]
  ...
  create-patients 1 [
    ...
    ;set contamCdiff patientcontamCdiff ;PLACE HOLDER
    ;set contamMRSA patientcontamMRSA
   ...
   ;set MRSApatienthandcontamCdiff 1 ;will update later
   ]
  ...
  create-nurses 1 [
    ...
    ;ask nurses-here [create-links-with other patients-here]
  ]
  ...
  create-wheelchairs 1 [
    set size 1
    set color gray
    set AWwheel random-float 51.65 + 154.8 ; place holder - maximim fraction of hand times maximum hand SA
    ;set AWhandle 0.17 * 535 ; place holder - maximum fraction of hand times maximum hand SA
  ...
  ask patients[
    ; SETTING UP C. DIFF CONTAM FOR PATIENT AND ASSOCIATED NURSE
    set AH random-float 90 + 445
    set contamCdiff random-float 5 + 1 / AH ; 1-6 spores per hand

    ;set contamCdiff patientcontamCdiff
    set transported? false

    ; SETTING UP MRSA CONTAM FOR PATIENT AND ASSOCIATED NURSE
    ...
    ;create-wheelpatients-with wheelchairs-here [tie]
    ;create-wheelpatients-with one-of wheelchairs-here [tie]

  ]
  ; ask nurses[
   ; ask one-of patients-here[
     ;ifelse contamCdiff > 0
     ; [set contamCdiff of myself random-float 5 + 1]
     ; [set contamCdiff of myself  0]]
    ;set contamCdiff random-float 10
   ; set contamMRSA random-float 10
    ;create-wheelnurses-with wheelchairs-here [tie]
 ; ]
  ; set handlescontamCdiff, armrestcontamCdiff, and occupied initial values for wheelchairs
  ; set up initial handcontamCdiff for patients and nurses
end

to go
  ask patients [transport]
  ;ask wheelchairs [nursecontaminate patientcontamCdiffinate]
  tick
end

to transport
  if transported? = false [
    ;ask one-of nurses-here [nursecontaminate]
  ...
  if location = "Out Patient Clinics" ; something wrong here w/ values - go and check Markvo chain code
...
[move-to one-of patches with [not any? patients-here and zone = "Common Spaces"]
      set transported? true]
      ;if prob.6 > 0.82 and prob.6 <= 0.58
      ;[let nextlocation "Domicilliary"]
...
[move-to one-of patches with [not any? patients-here and zone = "Out Patient Clinics"]
      set transported? true]
      ;if prob.6 > 0.81 and prob.5 <= 0.87
     ; [let nextlocation "Radiology"]
      if prob.6 > 32 / 33
[move-to one-of patches with [not any? patients-here and zone = "Specialty Care Services"]
      set transported? true]
    ]
  if location = "Specialty Care Services" ; something wrong here w/ values - go and check Markvo chain code
...
[move-to one-of patches with [not any? patients-here and zone = "Specialty Care Services"]
    ...
    ask one-of patients-here [patientcontaminatechair
     set finalcontamCdiff contamCdiff
      ;output-print finalcontamCdiff
     set finalcontammrsa contamMRSA
      ;output-print finalcontammrsa
      patientcontaminatesurf
       ; resetting patient params
        set AH random-float 90 + 445
        let contamnext random-float 1
        ifelse contamnext <= .5
      [
        set contamCdiff 0 ;assume next patients are not contam
        set contamMRSA  0 ;assume next patients are not contam
      ]
      [
...
to clean
  if cleaning = true[
    ask one-of wheelchairs-here [
      let cleantemp random-float 1
      ifelse cleantemp >= 0.5
      [
      ;cleaning MRSA
      set contamMRSA2 contamMRSA2
      set contamMRSA1 contamMRSA1
      set cleaned? FALSE]
      [
      ;cleaning C diff
...
;to nursecontaminate
;  let TCdiffhandle (random-float 1.5 + 5.4) / 100
;  let TMRSAhandle (random-float 0.1 + 3.9) / 100
;  let SHhandles random-float 0.07 + 0.10
;  ask one-of wheelchairs-here[
;    set contamCdiff2 contamCdiff2 - (TCdiffhandle * SHhandles * (contamCdiff2 - [contamCdiff] of myself)) * ([AH] of myself / AWhandle)
;    set contamMRSA2 contamMRSA2 - (TMRSAhandle * SHhandles * (contamMRSA2 - [contamMRSA] of myself)) * ([AH] of myself / AWhandle)
;    ask one-of nurses-here[
;        set contamCdiff contamCdiff - (TCdiffhandle * SHhandles * (contamCdiff - [contamCdiff2] of myself))
;        set contamMRSA contamMRSA - (TMRSAhandle * SHhandles * (contamMRSA - [contamMRSA2] of myself))]]
;   ifelse pcolor = black
;      []
;      [ifelse contamCdiff3 > 0
;      [set pcolor scale-color red contamCdiff3 10 0.4]
;      [set pcolor white]]
;end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count patches with [zone = \"Radiology\" and contamMRSA3 > 0]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count patches with [zone = \"Radiology\" and contamCdiff3 > 0]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean [contamMRSA1] of wheelchairs"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot max [contamMRSA3] of patches"
...
<experiments>
  <experiment name="experiment" repetitions="1000" runMetricsEveryStep="true">
    ...
    <go>go</go>
    <timeLimit steps="50"/>
    <metric>count wheelchairs with [cleaned? = TRUE]</metric>
