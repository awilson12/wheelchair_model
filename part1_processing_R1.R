...
setwd('C:/Users/wilso/Documents/Pubs/Accepted with revisions/Wheelchair pub/revision 1/revision output')
...
A<-ggplot(data=ribbondata[ribbondata$wheelchairtrips<10  & ribbondata$organism=="MRSA" & ribbondata$modeltype=="Scenario 1",])+
  geom_ribbon(aes(x=wheelchairtrips,ymin=mean-(1.960*sd/sqrt(1000)),ymax=mean+(1.960*sd/sqrt(1000)),group=efficacy,fill=efficacy),alpha=0.5)+
  geom_line(aes(x=wheelchairtrips,y=mean,group=efficacy,linetype=efficacy),size=1)+
  ...
  scale_fill_discrete(name="Efficacy")+
  theme_pubr()
...
D<-ggplot(data=ribbondata[ribbondata$wheelchairtrips<10 & ribbondata$modeltype=="Scenario 4" & ribbondata$organism=="MRSA",])+
  geom_ribbon(aes(x=wheelchairtrips,ymin=mean-(1.960*sd/sqrt(1000)),ymax=mean+(1.960*sd/sqrt(1000)),fill=efficacy),alpha=0.5)+
  geom_line(aes(x=wheelchairtrips,y=mean,group=efficacy,linetype=efficacy),size=1)+
  scale_y_continuous(trans="log10")+
  ylab(bquote('Contamination on Hands (CFU/cm '^2*')'))+
...
A<-ggplot(data=ribbondata[(ribbondata$modeltype=="Sensitivity Model 1" | ribbondata$modeltype=="Scenario 1 Primary Model") & ribbondata$wheelchairtrips<9 ,])+
  geom_line(aes(x=wheelchairtrips,y=mean,group=interaction(organism,efficacy),colour=efficacy,linetype=organism),size=1)+
  scale_y_continuous(trans="log10")+
  ylab(bquote(italic('  C. difficile')~ 'or MRSA Contamination on Patient Hands (CFU/cm '^2~')'))+
  scale_colour_discrete(name="Efficacy")+
  scale_linetype_discrete(name="Organism",labels=c(bquote(italic('C. difficile')),'MRSA'))+
  xlab("Number of Wheelchair Trips")+facet_wrap(~modeltype)+theme_pubr()+ggtitle("A")+
...
B<-ggplot(data=ribbondata.small)+
  ...
  scale_linetype_manual(values=c("solid", "dotted","dotdash","dashed"),(name="Efficacy"))+theme_pubr()+ggtitle("B")+
  theme(legend.position="right",legend.key.width = unit(1,"cm"))+
  guides(linetype = guide_legend(override.aes = list(size = 1)))
